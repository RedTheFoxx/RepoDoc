...
class TreeSitterPHPAnalyzer:
    ...
    def _extract_nodes(
        self, node, lines: List[str], depth: int = 0, parent_class: str = None
    ):
        ...
        if node.type == "class_declaration":
            ...
            if name_node:
                ...
                containing_class = parent_class or self._find_containing_class_name(
                    node
                )
                if containing_class:
    ...
    def _extract_relationships(self, node, depth: int = 0):
                ...
                for child in interface_clause.children:
                    if child.type in ("name", "qualified_name"):
                        ...
                        if not self._is_primitive(interface_name):
                            resolved_interface = self.namespace_resolver.resolve(
                                interface_name
                            )
                            self.call_relationships.append(
                                CallRelationship(
        ...
        if node.type == "object_creation_expression":
            ...
            type_node = self._find_child_by_type(
                node, "name"
            ) or self._find_child_by_type(node, "qualified_name")
            if type_node:
        ...
        if node.type == "scoped_call_expression":
            containing_class = self._find_containing_class_name(node)
            scope_node = self._find_child_by_type(
                node, "name"
            ) or self._find_child_by_type(node, "qualified_name")
        ...
        if node.type == "property_promotion_parameter":
            ...
            type_node = self._find_child_by_type(
                node, "type_list"
            ) or self._find_child_by_type(node, "named_type")
    ...
    def _extract_type_name(self, type_node) -> Optional[str]:
        ...
        if type_node.type == "named_type":
            name_node = self._find_child_by_type(
                type_node, "name"
            ) or self._find_child_by_type(type_node, "qualified_name")
            if name_node:
