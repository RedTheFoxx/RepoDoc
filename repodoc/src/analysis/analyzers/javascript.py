...
class TreeSitterJSAnalyzer:
    ...
    def analyze(self) -> None:
        ...
        try:
            ...
            logger.error(
                f"Error analyzing JavaScript file {self.file_path}: {e}", exc_info=True
            )
    ...
    def _get_component_id(
        self, name: str, class_name: str = None, is_method: bool = False
    ) -> str:
    ...
    def _create_method_node(
        self, node, method_name: str, class_name: str
    ) -> Optional[Node]:
        ...
        try:
            ...
            component_id = self._get_component_id(
                method_name, class_name, is_method=True
            )
            ...
            return Node(
                ...
                source_code="\n".join(
                    self.content.splitlines()[line_start - 1 : line_end]
                ),
    ...
    def _extract_call_from_node(
        ...
        try:
            ...
            if is_method_call:
                ...
                if current_class:
                    method_key = (
                        f"{self._get_module_path()}.{current_class}.{callee_name}"
                    )
        ...
        try:
            if hasattr(node, "prev_sibling") and node.prev_sibling:
                ...
                if prev.type == "comment":
                    ...
                    self._parse_jsdoc_types(
                        comment_text, caller_name, node.start_point[0] + 1
                    )
    ...
    def _parse_jsdoc_types(
        self, comment_text: str, caller_name: str, line_number: int
    ) -> None:
    ...
    def _extract_callee_name(self, call_node) -> Optional[str]:
        ...
        if callee_node.type == "identifier":
            ...
            computed_property = self._find_child_by_type(
                callee_node, "computed_property_name"
            )
...
def analyze_javascript_file_treesitter(
    ...
    try:
        ...
        logger.error(
            f"Error in tree-sitter JS analysis for {file_path}: {e}", exc_info=True
        )
        return [], []
