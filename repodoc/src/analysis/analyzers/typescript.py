...
class TreeSitterTSAnalyzer:
    ...
    def analyze(self) -> None:
        ...
        try:
            ...
            logger.error(
                f"Error analyzing TypeScript file {self.file_path}: {e}", exc_info=True
            )
        ...
        for entity_name, entity_data in all_entities.items():
            if self._is_actually_top_level(entity_data):
                ...
                if node_obj and self._should_include_node(node_obj):
                    ...
                    if entity_data["type"] in [
                        "class_declaration",
                        "abstract_class_declaration",
                    ]:
                        self._extract_constructor_dependencies(
                            entity_data["node"], entity_name
                        )
    ...
    def _get_parent_context(self, node) -> str:
        ...
        if parent_type in ["program", "source_file"]:
            ...
            if node.parent.parent and node.parent.parent.type in [
                "module",
                "ambient_declaration",
            ]:
                return "module_block"
    ...
    def _extract_function_entity(self, node, func_type: str, depth: int) -> dict:
        ...
        is_async = (
            "async" in code_snippet.split("function")[0]
    ...
    def _extract_arrow_function_entity(self, node, depth: int) -> dict:
        ...
        if parent and parent.type == "variable_declarator":
            ...
            if name_node:
                ...
                is_async = (
                    "async" in code_snippet.split("=")[0]
                    if "=" in code_snippet
                    else False
                )
                display_name = (
                    f"{'async ' if is_async else ''}arrow function {func_name}"
                )
    ...
    def _extract_variable_entity(self, node, depth: int) -> dict:
        ...
        has_function = self._find_child_by_type(
            node, "arrow_function"
        ) or self._find_child_by_type(node, "function_expression")
    ...
    def _extract_export_statement_entity(self, node, depth: int) -> dict:
        ...
        if func_decl:
            ...
            var_declarator = self._find_child_by_type(
                lexical_decl, "variable_declarator"
            )
            if var_declarator:
        ...
        try:
            ...
            for child in class_body.children:
                if child.type == "method_definition":
                    property_name = self._find_child_by_type(
                        child, "property_identifier"
                    )
                    if (
                        property_name
                        and self._get_node_text(property_name) == "constructor"
                    ):
                        # Extract parameter types
                        formal_params = self._find_child_by_type(
                            child, "formal_parameters"
                        )
                        if formal_params:
                            self._extract_parameter_dependencies(
                                formal_params, class_name
                            )
                        break
        except Exception as e:
                    ...
                    if type_annotation:
                        ...
                        if type_id:
                            ...
                            if dependency_name and dependency_name != caller_name:
                                ...
                                callee_id = (
                                    f"{self._get_module_path()}.{dependency_name}"
                                )
    ...
    def _traverse_for_relationships(
        ...
        if current_top_level:
            if node.type == "call_expression":
                ...
                self._extract_subscript_relationship(
                    node, current_top_level, all_entities
                )
                ...
                self._extract_type_arguments_relationship(
                    node, current_top_level, all_entities
                )

            elif node.type == "extends_clause":
                self._extract_inheritance_relationship(
                    node, current_top_level, all_entities
                )
    ...
    def _extract_call_relationship(
        ...
        try:
            ...
            if is_method_call:
                ...
                for entity_name, entity_data in all_entities.items():
                    if (
                        entity_data.get("type") == "class"
                        and caller_name in entity_name
                    ):
                ...
                if current_class and callee_name in all_entities:
                    ...
                    if (
                        callee_entity.get("subtype") == "method"
                        and callee_name in current_class
                    ):
                        return
        ...
        try:
            ...
            if node.children:
                ...
                if constructor_node:
                    ...
                    if constructor_name and not self._is_builtin_function(
                        constructor_name
                    ):
    ...
    def _extract_member_relationship(
        self, node, caller_name: str, all_entities: dict
    ) -> None:
        try:
            call_line = node.start_point[0] + 1
    ...
    def _extract_subscript_relationship(
        self, node, caller_name: str, all_entities: dict
    ) -> None:
        pass

    def _extract_type_relationship(
        self, node, caller_name: str, all_entities: dict
    ) -> None:
        try:
            type_identifiers = []
        ...
        try:
            for child in node.children:
                if child.type == "type_identifier":
                    ...
                    if type_name in all_entities:
                        target_name = self._resolve_to_top_level(
                            type_name, all_entities
                        )
                        if target_name and target_name in self.top_level_nodes:
                            call_line = node.start_point[0] + 1
                            self._add_relationship(caller_name, target_name, call_line)
    ...
    def _extract_inheritance_relationship(
        self, node, caller_name: str, all_entities: dict
    ) -> None:
        """Extract inheritance/implementation relationships"""
        try:
    ...
    def _add_relationship(
        self, caller_name: str, callee_name: str, call_line: int
    ) -> None:
        caller_id = f"{self._get_module_path()}.{caller_name}"
...
def analyze_typescript_file_treesitter(
    ...
    try:
        ...
        logger.error(
            f"Error in tree-sitter TS analysis for {file_path}: {e}", exc_info=True
        )
        return [], []
